...
check(len(b['TipsyTailTerrainCutoutSpec']['CutoutTiles'])==30 and all(v['Z']==2 for v in b['TipsyTailTerrainCutoutSpec']['CutoutTiles']),'Owned terrain cutout covers footprint at surface')
check('BuildingTerrainCutoutSpec' not in b and 'BlockObjectTerrainCutoutSpec' not in b,'No second native cutout owner or preview handler')
check((MOD/'Scripts/TipsyTail.Runtime.dll').is_file(),'Bundled terrain cleanup runtime is present')
