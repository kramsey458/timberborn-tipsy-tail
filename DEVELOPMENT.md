...
`validate_pool_water.py` reads the installed game's `Shaders.zip`, the exported pool model and `Source/Runtime/TipsyTailPoolWater.cs`. It confirms that the shader's connected UV nodes read UV1, that the exported pool surface has no UV1 while the runtime supplies one, that every shader property the C# touches exists, and that the UV scale sits inside the lake-to-fountain tiling ratios. It rewrites pool-water-validation.json.
