...
Changes in earlier versions are listed under [Earlier versions](#earlier-versions).
...
- When the water runs out, recreation and wet fur relief stop and the pool surface disappears. All three return when haulers resupply it.
- Visitors also satisfy the native Wet fur need, at the same 0.5 points per hour as the Lido and Swimming Pool.
- The game's own lake water in the pool: it darkens with depth, the floor and swimmers show through the shallows, and it ripples, glints and reflects like the map's still water.
- A campfire-style dirt-and-stakes construction site.
- Available to both Folktails and Iron Teeth under Well-being.

## Install

1. Download **TipsyTail-v1.0.1-mod.zip** from under **Assets** on the [release page](https://github.com/timbermods/timberborn-tipsy-tail/releases/tag/v1.0.1), not GitHub's automatic "Source code" archive. <!-- latest -->
...
To upgrade, replace the old TipsyTail folder with the new one. Keep only one version installed. No Unity editor, Blender installation, BepInEx or other mod is required to play. In multiplayer, every player installs the same version of the mod and runs the same game version.

## Optional water diagnostics

The pool water is the game's own, so there is nothing to tune. A `water.cfg` file in the mod folder (`Mods/TipsyTail`, or whatever you named it) is still read every few seconds without a restart. Download **TipsyTail-v1.0.1-water-presets.zip** from the release and copy one preset into the mod folder as `water.cfg`:

- the default game water, whose `log` line writes the pool's real mesh, material and shader values to Player.log;
- the v0.2.7 legacy ripples;
- the untouched vanilla fountain water.

Delete the file to return to the default. The file only changes how the pool is drawn. It does not affect gameplay or multiplayer.
...
| Unlock | 500 science |
| Visitors | Eight: four bar seats and four swimming lanes |
...
The drain continues without visitors, but the game's normal pause and blocking rules pause consumption. The surface has a fixed height, and this mod does not add water to the map simulation. No bartender job or custom drinking animation is included.
...
For a bug report, include your game and mod versions, faction, a screenshot and any relevant exception from `Player.log`. Remove personal information from logs before posting them publicly.
